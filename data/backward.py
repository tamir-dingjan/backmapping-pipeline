...
logging.basicConfig(filename="backward.log", level=logging.DEBUG)
##
...
def det(A):
    # print("\n### the value of A in det(A) function:", A)
    (a, d, g), (b, e, h), (c, f, k) = A
    return a * (e * k - f * h) - b * (k * d - f * g) + c * (d * h - e * g)


def m_inv(A):
    # print("\n### the value of A in m_inv(A) function:", list(A))
    u, v, w = A
    d = 1.0 / det(A)
...
def unbreak(R, box, inv=None):
    ...
    xyz = [
        vadd(R[0][4:7], mvmul(box, vr(mvmul(inv, vsub(i[4:7], R[0][4:7]))))) for i in R
    ]
...
def pdbOut(atom, i=1):
    ...
    return pdbline % (
        (i, atom[0][:4], atom[1][:4], atom[3], atom[2], 10 * x, 10 * y, 10 * z, 1, 0)
    )
...
class Structure:
    def __init__(self, other, strict=False):
        ...
        rest = []
        self.atoms = [
            pdbAtom(i, strict) for i in lines if isPDBAtom(i) or rest.append(i)
        ]
        if not self.atoms:
            # This should be a GRO file - get the atom count
            n = int(lines[1]) + 2  # This value points to the line with the box vectors
            # print("\n### atom count:", n)
            self.atoms = [groAtom(i) for i in lines[2:n]]
            b = [float(i) for i in lines[n].split()] + 6 * [
                0
            ]  # Padding for rectangular boxes
            # print("\n### the b list comprehension:", b)
            self.box = [
                [b[0], b[3], b[4]],
                [b[5], b[1], b[6]],
                [b[7], b[8], b[2]],
            ]  # Full definition xx,xy,xz,yx,yy,yz,zx,zy,zz
            # print("\n### self.box:", self.box)
        else:
        ...
        protein = [
            i[0][1].strip() in AminoAcids and get_calpha_xyz(i) for i in self.residues
        ]
        termini = [
            is_terminal(i, j, A, B)
            for i, j in zip([False] + protein, protein + [False])
        ]
...
class Topology:
    def __init__(self, other, out=None):
        ...
        self.atoms = [
            [a, r, i, c, 0, 0, 0, t]
            for t, c in mr
            for a, r, i, m in self.moleculetypes[t]
        ]
...
options = [
    ...
    (
        "-raw",
        Option(str, 1, None, "Projected structure before geometric modifications"),
    ),
    ...
    (
        "-po",
        Option(str, 1, None, "Output target topology with matching molecules list"),
    ),
...
# print("\n###########################")
# print("\n### The contents of options[atomlist]: ", options["-atomlist"], options["-atomlist"].value)
...
if top:
    ...
    # print("\n### value of options['-atomlist']:", options["-atomlist"], options["-atomlist"].value)
    # print("\n### bool evaluation of options['-atomlist']:", bool(options["-atomlist"].value))
    if options["-atomlist"].value:
        ...
        atm.writelines(
            "".join(
                [
                    "%6d %5s %5s\n" % (u, v[0], v[1])
                    for u, v in zip(range(1, len(topatm) + 1), topatm)
                ]
            )
        )
...
sol = []
ions = []
for residue, bb, nterm, cterm in zip(
    struc.residues, struc.backbone, struc.nterm, struc.cterm
):
    ...
    if topresidues:
        ...
        if not len(target) == len(set(target)):
            logging.WARNING(
                "The target list for residue %s contains duplicate names. Relying on mapping file."
                % resn
            )
            target = None
    else:
    ...
    # print("### About to map residue: ", resn, "\n\n", residue, "\n\n", target)
...
for atom in out:
    ...
    # The kick routine is originally disabled behind an "if False"
    # if False and res not in solvent_stuff:
    # print(res)
    if res not in solvent_stuff:
        # print(x,y,z,"\t",kick(x,u),kick(y,u),kick(z,u))
...
if options["-p"].value and options["-po"].value:
